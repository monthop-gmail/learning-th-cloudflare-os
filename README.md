...
3. `ask_cfos_agent` ทำไว้ให้แล้วใน `cfos-context-mcp/server.mjs` (พร้อม `list_cfos_workspaces`
   และ `check_cfos_chat`) — อ่านโค้ดแล้วตอบว่ามันบังคับกติกา "ห้ามสร้าง workspace ใหม่"
   ด้วยวิธีไหน และถ้าบัญชีมีสิทธิ์แค่ `use` จะเกิดอะไรขึ้น
